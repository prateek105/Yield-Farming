...
contract('YnoteSeedMaster', ([alice, bob, carol, dev, eliah, minter, feeAddress]) => {
    beforeEach(async () => {
    ...
    it('should set correct state variables', async () => {
        this.master = await YnoteSeedMaster.new(this.YnoteToken.address, 100, 5,505, feeAddress, { from: alice })
        await this.YnoteToken.setMaster(this.master.address, { from: alice })
        
        const yNote = await this.master.yNote()
        assert.equal(yNote.valueOf(), this.YnoteToken.address)
    ...
    it('should allow only master farmer can mint', async () => {
        this.master = await YnoteSeedMaster.new(this.YnoteToken.address, 1234, 100,500, feeAddress, { from: alice })
    ...
    context('With LP token added to the field', () => {
        ...
        it('should correct add new pool and set pool', async () => {
            ...
            this.master = await YnoteSeedMaster.new(this.YnoteToken.address, 100, 100,500, feeAddress, { from: alice })
            await this.YnoteToken.setMaster(this.master.address, { from: alice })
        ...
        it('should give out YnoteToken only after end of staking period', async () => {
            ...
            await expectRevert(
                ...
                "YnoteSeedMaster: No tokens staked"
            )
            await expectRevert(
                this.master.harvest(0, { from: bob }), 
                "YnoteSeedMaster: No rewards to claim"
            )
            await expectRevert(
                this.master.deposit(0, 100,constants.ZERO_ADDRESS, { from: bob }),
                "YnoteSeedMaster: Staking period has ended"
         )
            assert.equal((await this.YnoteToken.balanceOf(bob)).valueOf(), '252')
        ...
        it('should not distribute YnoteToken if no one deposit', async () => {
            ...
            this.master = await YnoteSeedMaster.new(this.YnoteToken.address, 10, 400,500,feeAddress, { from: alice })
            await this.YnoteToken.setMaster(this.master.address, { from: alice })
            ...
            await this.master.harvest(0, { from: bob })
            assert.equal(await this.YnoteToken.balanceOf(bob).valueOf(),'400')
        ...
        it('should equally distribute', async () => {
           this.master = await YnoteSeedMaster.new(this.YnoteToken.address, 10, 600,700,feeAddress, { from: alice })
           await this.YnoteToken.setMaster(this.master.address, { from: alice })
          ...
          // ----- claiming anytime after sale end (equal distribution)
            ...
            assert.equal(await this.YnoteToken.balanceOf(alice),'130');

            await this.master.harvest(0, { from: bob })
            assert.equal(await this.YnoteToken.balanceOf(bob),'126');

            await this.master.harvest(0, { from: carol })
            assert.equal(await this.YnoteToken.balanceOf(carol),'123');

            await this.master.harvest(0, { from: dev })
            assert.equal(await this.YnoteToken.balanceOf(dev),'121');

            await this.master.harvest(1, { from: eliah })
            assert.equal(await this.YnoteToken.balanceOf(eliah),'479');           
        ...
        it('should properly distribute at different deposit amounts', async () => {
            this.master = await YnoteSeedMaster.new(this.YnoteToken.address, 10, 800,900,feeAddress, { from: alice })
            await this.YnoteToken.setMaster(this.master.address, { from: alice })
           ...
           // ----- claiming anytime after sale end (equal distribution)
             ...
             await this.master.harvest(0, { from: alice })
             assert.equal(await this.YnoteToken.balanceOf(alice),'55');
             await this.master.harvest(0, { from: bob })
             assert.equal(await this.YnoteToken.balanceOf(bob),'102');
             await this.master.harvest(0, { from: carol })
             assert.equal(await this.YnoteToken.balanceOf(carol),'148');
             await this.master.harvest(0, { from: dev })
             assert.equal(await this.YnoteToken.balanceOf(dev),'194');
             await this.master.harvest(1, { from: eliah })
             assert.equal(await this.YnoteToken.balanceOf(eliah),'479');
        ...
        it('should distribute properly when multiple deposit', async () => {
            this.master = await YnoteSeedMaster.new(this.YnoteToken.address, 10, 1000,1100,feeAddress, { from: alice })
            await this.YnoteToken.setMaster(this.master.address, { from: alice })
        ...
        it('should allow partial withdraw but not give Ynote token', async () => {
            this.master = await YnoteSeedMaster.new(this.YnoteToken.address, 10, 1200,1300,feeAddress, { from: alice })
            await this.YnoteToken.setMaster(this.master.address, { from: alice })
        ...
        it('should pay to referrer address if a user is referred by it', async () => {
            ...
            await this.YnoteToken.setMaster(this.master.address, { from: alice })
             ...
             assert.equal(await this.YnoteToken.balanceOf(alice),'252'); 
             assert.equal(await this.YnoteToken.balanceOf(bob),'247'); 
             assert.equal(await this.YnoteToken.balanceOf(carol),'5'); 
        }) 

        
        it('should not be referred by multiple referrers', async () => {
            this.master = await YnoteSeedMaster.new(this.YnoteToken.address, 10, 1600,1700,feeAddress, { from: alice })
            await this.YnoteToken.setMaster(this.master.address, { from: alice })
            ...
            assert.equal(await this.YnoteToken.balanceOf(alice),'293'); 
            assert.equal(await this.YnoteToken.balanceOf(bob),'205'); 
            assert.equal(await this.YnoteToken.balanceOf(carol),'5');
            assert.equal(await this.YnoteToken.balanceOf(dev),'0');         
        }) 

        it('should allow withdraw after change of master address in token contract', async () => {
            this.master = await YnoteSeedMaster.new(this.YnoteToken.address, 10, 1800,1900,feeAddress, { from: alice })
            await this.YnoteToken.setMaster(this.master.address, { from: alice })
            ...
            await this.master.withdraw(0,3, { from: alice })
             ...
             await this.master.harvest(0, { from: bob })
             assert.equal(await this.YnoteToken.balanceOf(bob),'125');
             ...
             await this.master.harvest(1, { from: eliah })
             assert.equal(await this.YnoteToken.balanceOf(eliah),'480');           
         ...
         it('should allow change of deposit fee', async () => {
            this.master = await YnoteSeedMaster.new(this.YnoteToken.address, 10, 2000,2100,feeAddress, { from: alice })
            await this.YnoteToken.setMaster(this.master.address, { from: alice })
             ...
             await this.master.harvest(0, { from: alice })
             assert.equal(await this.YnoteToken.balanceOf(alice),'194');
 
             await this.master.harvest(0, { from: bob })
             assert.equal(await this.YnoteToken.balanceOf(bob),'189');         

             await this.master.harvest(0, { from: carol })
             assert.equal(await this.YnoteToken.balanceOf(carol),'58');
 
             await this.master.harvest(0, { from: dev })
             assert.equal(await this.YnoteToken.balanceOf(dev),'57');   

             assert.equal((await this.lp.balanceOf(feeAddress)).valueOf(), '20')
        })

        it('should allow original fee address to change feeAddress', async () => {
            this.master = await YnoteSeedMaster.new(this.YnoteToken.address, 10, 2200,2300,feeAddress, { from: alice })
            await this.YnoteToken.setMaster(this.master.address, { from: alice })
            await expectRevert(
            ...
            assert.equal(await this.YnoteToken.balanceOf(alice),'252'); 
            assert.equal(await this.YnoteToken.balanceOf(bob),'248'); 
            assert.equal(await this.YnoteToken.balanceOf(carol),'5');
            assert.equal(await this.YnoteToken.balanceOf(dev),'0');
